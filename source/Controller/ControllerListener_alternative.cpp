...
ControllerListenerAlternative::ControllerListenerAlternative(HANDLE hEvent,
                                                             HANDLE hCtr,
                                                             PZG_CTR_INFO pInfo,
...
ControllerListenerAlternative::~ControllerListenerAlternative()
{
//    delete this->pWrIndx;
//    delete this->pRdIdx;
...
bool ControllerListenerAlternative::getBThreadActive() const
{
    return bThreadActive;
}

void ControllerListenerAlternative::setBThreadActive(bool newBThreadActive)
{
    bThreadActive = newBThreadActive;
}

void ControllerListenerAlternative::run()
{
    ...
    if(Result != S_OK)
    {
        qDebug()<<" {ControllerListenerAlternative} ZG_Ctr_WriteLockTimes  - Ошибка в установке времени открытия замка";
        throw std::exception("{ControllerListenerAlternative} ZG_Ctr_WriteLockTimes");
    }
...
//        this->CheckNotifyMsgs();
        qDebug()<<"CheckNotifyMsgsAlternative="<<this->rCtrInfo->nSn;
        QElapsedTimer TimeEvent;
        TimeEvent.start();
        CheckNotifyMsgsAlternative();
        qDebug() << "CheckNotifyMsgsAlternative - Время выполнения функции: " <<TimeEvent.elapsed() << " миллисекунд";
        qDebug() << "Thead: " <<this->currentThread();
        qDebug() << "";

        if(!this->database->getActiveBySN(this->rCtrInfo->nSn))
        {
...
void ControllerListenerAlternative::SendAndPrint(QJsonObject DataToSend)
{
...
void ControllerListenerAlternative::CheckNotifyMsgsAlternative(){
...
//            this->LastKeyCard  = keycard;
            ...
            qDebug()<< " {Class ControllerListenerAlternative} -  Карта старая совпадает с новой  ";
...
void ControllerListenerAlternative::ShowEvents(
...
void ControllerListenerAlternative::Processing_Event_Button_Open(PZG_CTR_EVENT pEv)
{
            _ZG_EV_TIME rTime;
...
void ControllerListenerAlternative::Processing_Event_Key(PZG_CTR_EVENT pEv)
{
            _ZG_EV_TIME rTime;
            ZG_CTR_DIRECT nDirect;
            INT nKeyIdx, nKeyBank;
...
void ControllerListenerAlternative::Processing_Event_Unknown_Key(PZG_CTR_EVENT pEv)
{
...
void ControllerListenerAlternative::Processing_Event_Mode_State(PZG_CTR_EVENT pEv)
{
            _ZG_EV_TIME rTime;
            ZG_CTR_MODE nMode;
...
void ControllerListenerAlternative::Processing_Event_Change_Electro(PZG_CTR_EVENT pEv)
{
    _ZG_EV_TIME rTime;
...
void ControllerListenerAlternative::Processing_Event_Fire_State(PZG_CTR_EVENT pEv)
{
    _ZG_EV_TIME rTime;
